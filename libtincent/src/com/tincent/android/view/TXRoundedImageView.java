...
public class TXRoundedImageView extends ImageView {
	...
	private ColorStateList borderColor = ColorStateList
			.valueOf(TXRoundedDrawable.DEFAULT_BORDER_COLOR);
	...
	public TXRoundedImageView(Context context) {
		super(context);
	}

	public TXRoundedImageView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public TXRoundedImageView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);

		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.TXRoundedImageView, defStyle, 0);

		int index = a.getInt(R.styleable.TXRoundedImageView_android_scaleType,
				-1);
		if (index >= 0) {
		...
		cornerRadius = a.getDimensionPixelSize(
				R.styleable.TXRoundedImageView_riv_corner_radius, -1);
		borderWidth = a.getDimensionPixelSize(
				R.styleable.TXRoundedImageView_riv_border_width, -1);
		...
		borderColor = a
				.getColorStateList(R.styleable.TXRoundedImageView_riv_border_color);
		if (borderColor == null) {
		...
		mutateBackground = a.getBoolean(
				R.styleable.TXRoundedImageView_riv_mutate_background, false);
		isOval = a.getBoolean(R.styleable.TXRoundedImageView_riv_oval, false);

		final int tileMode = a.getInt(
				R.styleable.TXRoundedImageView_riv_tile_mode,
				TILE_MODE_UNDEFINED);
		...
		final int tileModeX = a.getInt(
				R.styleable.TXRoundedImageView_riv_tile_mode_x,
				TILE_MODE_UNDEFINED);
		...
		final int tileModeY = a.getInt(
				R.styleable.TXRoundedImageView_riv_tile_mode_y,
				TILE_MODE_UNDEFINED);
	...
	public void setImageDrawable(Drawable drawable) {
		mResource = 0;
		mDrawable = TXRoundedDrawable.fromDrawable(drawable);
		updateDrawableAttrs();
	...
	public void setImageBitmap(Bitmap bm) {
		mResource = 0;
		mDrawable = TXRoundedDrawable.fromBitmap(bm);
		updateDrawableAttrs();
		super.setImageDrawable(mDrawable);
	}
	...
	private Drawable resolveResource() {
		...
		return TXRoundedDrawable.fromDrawable(d);
	}
	...
	private void updateBackgroundDrawableAttrs(boolean convert) {
		if (mutateBackground) {
			if (convert) {
				mBackgroundDrawable = TXRoundedDrawable
	...
	private void updateAttrs(Drawable drawable) {
		...
		if (drawable instanceof TXRoundedDrawable) {
			((TXRoundedDrawable) drawable).setScaleType(mScaleType)
